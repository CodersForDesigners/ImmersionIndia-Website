 ...
 */
// Packages
$db_file_name = $_SERVER[ 'DOCUMENT_ROOT' ] . '/database/package_database.json';
...
// Schedules
$directory_schedules = $_SERVER[ 'DOCUMENT_ROOT' ] . '/uploads/schedules/';
...
function validateTinyPNG_APIKey () {
	try {
		...
		logThis( 'packages', 'Attempting to validate TinyPNG\'s API key', [ 'type' => 'error', 'message' => $message ] );
		return false;
	}
	return true;
}
...
function resizeAndCompressImage ( $file_name, $source_dir, $target_dir ) {
	 ...
	 */
	try {
		$processor = \Tinify\fromFile( $inputFile );
		...
		$message = $e->getMessage();
		logThis( 'packages', 'Attempting to process the file using TinyPNG', [ 'type' => 'error', 'file' => $file_name, 'message' => $message ] );
		return false;
	}
...
function processImages ( $source_directory, $file_names, $target_directory ) {
	...
	foreach ( $file_names as $name ) {
		...
		$processed_image_names[ ] = $name;
	}

	return $processed_image_names;
...
if ( $_POST[ 'action' ] == 'create' ) {
	...
	$location_files = getRestructuredObject( $_FILES[ 'locations' ] ?? [ ] );
	$schedule_file = [ ];
	if ( ! empty( $_FILES[ 'schedule' ] ) ) {
		$schedule_file[ ] = $_FILES[ 'schedule' ];
	}
	...
	$location_files_name_map = getFormattedFilenames( $location_files );
	$schedule_file_name_map = getFormattedFilenames( $schedule_file );

	// reflect the actual file-names in the database
	foreach ( $data[ 'locations' ] as &$place ) {
		$source = $place[ 'image' ];
		$place[ 'image' ] = $location_files_name_map[ $source ] ?? $source;
	}
	unset( $place );
	$data[ 'schedule' ] = $schedule_file_name_map[ $data[ 'schedule' ] ] ?? $data[ 'schedule' ];
	...
	// pull in any files ( if any were sent along )
	moveAndRenameFiles( $location_files, $location_files_name_map, $directory_temporary );
	moveAndRenameFiles( $schedule_file, $schedule_file_name_map, $directory_schedules );

	// compress the images
	processImages( $directory_temporary, $location_files_name_map, $directory_processed );

	// delete the originally uploaded location files
	foreach ( $location_files_name_map as $name ) {
...
/* -----
 ...
 ----- */
	...
	$images_from_db = array_column( $db[ $index ][ 'locations' ], 'image' );
	if ( ! empty( $db[ $index ][ 'schedule' ] ) ) {
		$images_from_db[ ] = $db[ $index ][ 'schedule' ];
	}
	$images_from_request = array_column( $data[ 'locations' ], 'image' );
	if ( ! empty( $data[ 'schedule' ] ) ) {
		$images_from_request[ ] = $data[ 'schedule' ];
	}
	...
	// delete the files that belong to the diff that we computed from earlier
	foreach ( $diff as $name ) {
		@unlink( $directory_processed . $name );
		@unlink( $directory_processed . 'thumbnails/' . $name );
		@unlink( $directory_processed . 'schedules/' . $name );
...
/* -----
 ...
 ----- */
	...
	// delete the files associated with the package
	$files = array_column( $db[ $index ][ 'locations' ], 'image' );
	if ( ! empty( $db[ $index ][ 'schedule' ] ) ) {
		$files[ ] = $db[ $index ][ 'schedule' ];
	}
